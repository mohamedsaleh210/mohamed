...
- Everything except the `erase` group by role default, but **cannot**
  ...
  by a Super Admin — this per-user revocation mechanism was verified
  working end to end during this platform's own testing (real login,
  real 403 on the revoked action, other abilities unaffected).
