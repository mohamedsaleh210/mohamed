...
async function copySnapshot(source, destination) {
  ...
  // PLATFORM_ROOT is owned by a separate central-platform application and
  // only exists on a real multi-tenant host — a full-scope backup taken
  // anywhere else should still produce a valid (office-only-equivalent)
  // archive rather than crash the request.
