...
function create(req,input,options={}){return db.transaction(()=>{
 ...
 // A guest or client booking online has no acting staff member to inherit
 // a branch from — both the request and its agenda entry land on the
 // office's main branch, same as any other self-service submission.
 const officeBranchId=require('./office-branches').mainBranchId();
 const r=db.prepare('INSERT INTO requests(name,phone,email,client_id,service_id,service_label,message,is_custom,ref,upload_token,office_branch_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)').run(client.full_name,client.phone,client.email,client.id,service?.id||null,service?.title_ar||'حجز موعد',notes,service?0:1,require('./ref').generate(c=>!!db.prepare('SELECT 1 FROM requests WHERE ref=?').get(c)),crypto.randomBytes(24).toString('hex'),officeBranchId);
 ...
 const a=db.prepare("INSERT INTO agenda_events(title,event_type,starts_at,ends_at,request_id,client_id,client_visible,location,notes,office_branch_id) VALUES(?,'booking',?,?,?,?,1,?,?,?)").run(ref,s.starts_at,s.ends_at,rid,client.id,s.location,notes,officeBranchId);
