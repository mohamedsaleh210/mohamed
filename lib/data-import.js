...
/** A rejected row's own error text already says why (missing field, bad format,
 * duplicate-in-file, duplicate-in-system) — this only labels which of those
 * are duplicates, for the summary counter. It never changes what gets stored
 * per row or which rows are rejected. */
const isDuplicateError = (e) => e.includes('مكرر داخل الملف') || e.includes('موجود مسبقًا في النظام');

function commitBatch(batch, req) {
  if(batch.status!=='review')throw new Error('تم تنفيذ عملية الاستيراد من قبل');
  const seen={};
  // Re-validated fresh at commit time (not reused from the preview pass) so that
  // both in-file duplicates and duplicates against the database reflect the
  // system's current state, and so a row that only became invalid between
  // preview and commit (a race, not a stale-preview edge case) is caught and
  // rejected on its own rather than blocking the rest of the file.
  ...
  const good=checked.filter(x=>!x.errors.length);
  const duplicates=bad.filter(x=>x.errors.some(isDuplicateError)).length;
  const quotaKind={employees:'users',requests:'requests',branches:'branches'}[batch.entity];
  if(quotaKind&&good.length){const adding=good.filter(x=>x.data.active!==0).length,quota=tenantPolicy.allowance(quotaKind,adding);if(!quota.allowed)throw new Error('تجاوز ملف الاستيراد حد الباقة المسموح. خفّض عدد الصفوف أو اطلب ترقية الباقة.');}
  const credentials=[];const refs=[];
  const run=db.transaction(()=>{
    // Only rows that passed validation are ever inserted — rejected rows are
    // never written, partially or otherwise; they are reported back, not stored.
    for(const item of good){const r=item.data;let info,id;
    ...
    const rejectedRows=bad.map(x=>({rowNumber:x.rowNumber,errors:x.errors}));
    // count kept as an alias of imported for any older caller reading .count —
    // both are always the same number, imported is just the explicit name.
    const result={total:checked.length,imported:good.length,rejected:bad.length,duplicates,count:good.length,credentials,refs,rejectedRows};
    const storedResult={total:checked.length,imported:good.length,rejected:bad.length,duplicates,count:good.length,refs,rejectedRows};
    db.prepare(`UPDATE data_import_batches SET status='completed',imported_at=datetime('now'),result_json=?,payload_json='[]',valid_rows=?,error_rows=?,errors_json=? WHERE id=?`)
      .run(JSON.stringify(storedResult),good.length,bad.length,JSON.stringify(checked.map(x=>({rowNumber:x.rowNumber,errors:x.errors}))),batch.id);
    return result;
  });
