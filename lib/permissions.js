 ...
 *
 * A Super Admin is deliberately outside the system. Someone has to be able to
 * fix a misconfiguration, and that someone cannot be locked out by it. A
 * regular admin is not: it is a role like any other, its defaults happen to
 * start at everything, and a Super Admin can trim it down like any other
 * role's exceptions. `is_super_admin` is a column on the `users` table, not a
 * fifth role string — every existing `role === 'admin'` check (route gates,
 * the Sanad-owner check, the demo seed) still means "either tier of admin"
...
const CATALOGUE = {
  ...
  admin: {
    ...
    items: {
      ...
      'imports.manage': {
        label: 'يستورد بيانات بالجملة من Excel',
        note: 'كان مقصورًا على دور الأدمن مباشرة — بقى صلاحية قابلة للتحديد.',
      },
      'content_protection.manage': {
        label: 'يدير حماية محتوى الموقع العام',
        note: 'تفعيل حماية النسخ والعلامة المائية على الصفحات العامة — رادع، مش قفل أمني حقيقي.',
      },
    },
  },
};
 ...
 *
 * `admin` defaults to everything EXCEPT the `erase` group — same rule every
 * other role already follows here: destroying records is granted to a
 * person, never inherited from a job title, admin included. An admin account
 * that already had erase access the day Super Admin shipped keeps it, but as
 * an explicit per-person grant recorded in `user_permissions` (see the
 * migration bootstrap in seed.js), not as a role default a brand-new admin
 * would also silently get. Super Admin itself is absent from this table on
 * purpose — see `isSuperAdmin`/`resolve` below, it is not a set of
 * permissions, it is the absence of a limit.
 */
const ERASE_KEYS = Object.keys(CATALOGUE.erase.items);
const ROLE_DEFAULTS = {
  admin: ALL.filter((k) => !ERASE_KEYS.includes(k)),
...
/** True for either tier of admin — unrelated to what they may configure. */
const isAdmin = (user) => !!user && user.role === 'admin';

/**
 * True only for the protected tier: unconditional access, immune to having
 * permissions stripped through the normal UI, the only tier that may
 * configure a regular admin's permissions.
 */
const isSuperAdmin = (user) => !!user && user.role === 'admin' && !!user.is_super_admin;
 ...
 * always works, whatever their role says. Super Admin skips all of this —
 * see `resolve` — a regular admin does not: its defaults happen to start at
 * `ALL`, but it goes through the same override path as any other role.
 */
...
function resolve(db, user) {
  if (!user) return new Set();
  if (isSuperAdmin(user)) return new Set(ALL);
